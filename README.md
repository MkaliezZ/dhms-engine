...
| --- | --- | --- |
| [LangGraph](docs/dhms_agentfuse_five_minute_integration_trial_v3_7_1.md) | Packaged `LangGraphRuntimeGuardAdapter` for an explicit `ToolNode` path, with identity-preserving terminal results and tested allow/block behavior at recorded versions. Adapter dispatch means the host continuation was invoked; `host_completed` means that opaque continuation returned successfully while physical handler state remains `unknown`. | Built-in public-beta adapter; not universal LangGraph interception. |
