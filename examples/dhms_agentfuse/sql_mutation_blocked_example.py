#!/usr/bin/env python3
"""DHMS AgentFuse SQL mutation blocked example.

This example represents mutation-like SQL as inert data. It does not execute
SQL, import sqlite3, create SQLite databases, or create sandbox files.
"""
...
MUTATION_SQL = "DELETE FROM toy_accounts WHERE id = 1;"
...
def run_example() -> dict[str, Any]:
    request = create_runtime_request(
        request_id="example_request_sql_mutation_blocked",
        source="dhms_agentfuse_protocol_example",
        intent_summary="Review mutation SQL proposal without execution",
        raw_event={"event_type": "tool_call_proposal", "observed_only": True},
        metadata={"example_id": "sql_mutation_blocked"},
    )
    proposal = create_tool_call_proposal(
        proposal_id="example_proposal_sql_mutation_blocked",
        request_id=request.request_id,
        tool_name="sql.query",
        tool_type="SQL",
        requested_effect="write_database",
        payload={"sql": MUTATION_SQL},
        metadata={"example_id": "sql_mutation_blocked"},
    )
    ...
    if decision.decision not in {"BLOCK", "FAIL_CLOSED"}:
        failed_checks.append("mutation_not_blocked_or_fail_closed")
    if decision.release_eligible:
        failed_checks.append("mutation_release_eligible")
    ...
    return {
        "example_name": "DHMS AgentFuse SQL Mutation Blocked Example",
        "example_id": "sql_mutation_blocked",
        "status": status,
        "request_id": request.request_id,
        ...
        "final_verdict": (
            ...
            if status == "PASS"
            else "SQL_MUTATION_BLOCKED_EXAMPLE_FAIL"
        ),
    }
