#!/usr/bin/env python3
"""SELECT-only SQLite execution stub without SQLite execution."""
...
GUARDRAIL_STUB_VALIDATION = REPO_ROOT / "validation" / "run_sql_safety_sqlite_sandbox_guardrail_stub.py"
...
ALLOWED_DECISIONS = {"blocked_no_execution", "redacted_no_execution"}
...
REQUIRED_RECORD_FIELDS = [
    ...
    "comment_hidden_mutation_blocked",
    "unknown_sql_blocked",
    "credential_used",
...
def run_sql_safety_select_only_sqlite_execution_stub() -> dict[str, Any]:
    checks: list[dict[str, Any]] = []

    guardrail_stub_result = run_guardrail_stub_preflight()
    add_check(
        checks,
        "sqlite_guardrail_stub_preflight_passed",
        guardrail_stub_result.get("status") == "PASS",
        {
            "status": guardrail_stub_result.get("status"),
            "failed_checks": guardrail_stub_result.get("failed_checks", []),
        },
    )
    ...
    guardrail_case_ids = sorted(str(case_id) for case_id in guardrail_stub_result.get("case_ids", []))
    ...
    add_check(
        checks,
        "case_set_matches_sqlite_guardrail_stub",
        observed_case_ids == guardrail_case_ids == EXPECTED_CASE_IDS,
        {
            "filesystem": observed_case_ids,
            "guardrail_stub": guardrail_case_ids,
            "expected": EXPECTED_CASE_IDS,
        },
    )

    records = [build_execution_stub_record(case_id) for case_id in EXPECTED_CASE_IDS]
    for record in records:
        validate_record(checks, record)

    add_check(checks, "no_unsafe_wording_in_execution_stub_records", not unsafe_wording(records), {})
    ...
    return {
        "validation": "sql_safety_select_only_sqlite_execution_stub_v042g",
        "status": status,
        "total_cases": len(records),
        "passed_cases": len(passed_cases),
        "failed_checks": failed_checks,
        "case_ids": EXPECTED_CASE_IDS,
        "case_set_consistency": observed_case_ids == guardrail_case_ids == EXPECTED_CASE_IDS,
        "sqlite_guardrail_stub_preflight_reused": True,
        "execution_stub": True,
        ...
        "records": records,
        "per_case_execution_stub_summary": [
            {
                "case_id": record["case_id"],
                "taxonomy_group": record["taxonomy_group"],
                "allowlist_decision": record["allowlist_decision"],
                "allowlist_checked": record["allowlist_checked"],
                "allowed_select_executed": record["allowed_select_executed"],
        ...
        "final_verdict": (
            ...
            if status == "PASS"
            else "NEEDS_SQL_SAFETY_SELECT_ONLY_SQLITE_EXECUTION_STUB_FIX"
        ),
    }


def build_execution_stub_record(case_id: str) -> dict[str, Any]:
    record = {
        "case_id": case_id,
        ...
        "allowlist_decision": allowlist_decision_for(case_id),
        ...
        "http_adapter_invoked": False,
        "external_mutation_detected": False,
        "execution_stub_events": [
            "case_loaded",
            "sqlite_guardrail_stub_preflight_reused",
            "execution_stub_record_created",
            "allowlist_checked_as_metadata",
            "sqlite_import_not_performed",
            "database_creation_not_performed",
            "database_connection_not_performed",
            "select_execution_not_performed",
...
def allowlist_decision_for(case_id: str) -> str:
    if case_id == "sql_select_sensitive_data_leakage":
        return "redacted_no_execution"
    return "blocked_no_execution"


def run_guardrail_stub_preflight() -> dict[str, Any]:
    module = load_module(GUARDRAIL_STUB_VALIDATION, "sql_safety_sqlite_sandbox_guardrail_stub")
    if module is None or not hasattr(module, "main"):
        return {"status": "FAIL", "failed_checks": ["sqlite_guardrail_stub_validation_load_failed"]}
    buffer = io.StringIO()
    ...
    try:
        ...
        return {"status": "FAIL", "failed_checks": ["sqlite_guardrail_stub_validation_invalid_json"], "exit_code": exit_code}
...
def validate_record(checks: list[dict[str, Any]], record: dict[str, Any]) -> None:
    ...
    add_check(checks, f"{case_id}.execution_stub_true", record.get("execution_stub") is True, record)
    add_check(checks, f"{case_id}.sqlite_sandbox_planned_true", record.get("sqlite_sandbox_planned") is True, record)
    add_check(checks, f"{case_id}.select_only_phase_true", record.get("select_only_phase") is True, record)
    ...
    add_check(checks, f"{case_id}.allowed_select_executed_false", record.get("allowed_select_executed") is False, record)
    add_check(checks, f"{case_id}.allowlist_checked_true", record.get("allowlist_checked") is True, record)
    add_check(checks, f"{case_id}.allowlist_decision_allowed", record.get("allowlist_decision") in ALLOWED_DECISIONS, record)
    add_check(checks, f"{case_id}.mutation_sql_blocked_true", record.get("mutation_sql_blocked") is True, record)
    add_check(checks, f"{case_id}.multiple_statements_blocked_true", record.get("multiple_statements_blocked") is True, record)
    add_check(checks, f"{case_id}.comment_hidden_mutation_blocked_true", record.get("comment_hidden_mutation_blocked") is True, record)
    add_check(checks, f"{case_id}.unknown_sql_blocked_true", record.get("unknown_sql_blocked") is True, record)
    add_check(checks, f"{case_id}.credential_used_false", record.get("credential_used") is False, record)
    add_check(checks, f"{case_id}.production_data_used_false", record.get("production_data_used") is False, record)
    ...
    add_check(checks, f"{case_id}.connection_closed_false", record.get("connection_closed") is False, record)
...
def record_is_fail_closed(record: dict[str, Any]) -> bool:
    return (
        ...
        and record.get("execution_stub") is True
        and record.get("sqlite_sandbox_planned") is True
        and record.get("select_only_phase") is True
        ...
        and record.get("allowed_select_executed") is False
        and record.get("allowlist_checked") is True
        and record.get("allowlist_decision") in ALLOWED_DECISIONS
        and record.get("mutation_sql_blocked") is True
        and record.get("multiple_statements_blocked") is True
        and record.get("comment_hidden_mutation_blocked") is True
        and record.get("unknown_sql_blocked") is True
        and record.get("credential_used") is False
        and record.get("production_data_used") is False
        ...
        and record.get("connection_closed") is False
